...
public class SPuzzle {
    ...
    private void lockPredefinedCells() {
        for (int row = 0; row < grid.getRowCount(); row++) {
            for (int col = 0; col < grid.getColumnCount(); col++) {
                SCell cell = grid.getCell(row, col);
                if (!cell.isEmpty()) {
                    cell.setLocked(true);
                }
            }
     ...
     *   (see {@link ypa.model.SPuzzle#isValidNumber(int)})</li>
    ...
    public static String makeEmptyDescriptor(int size) {
        if (size <= 0) {
            throw new IllegalArgumentException(SPuzzle.class.getSimpleName()
                    + ".makeEmptyDescriptor().pre failed: "
                    + "size == " + size + " < 0");
        }
        return "Undefined Sujiko puzzle state with size " + size;
    }
